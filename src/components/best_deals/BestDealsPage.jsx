...
import { bestDealsCountriesPage } from "../../utils/data";
import axios from "axios";
import { BASE_URL } from "../../utils/constants";
import { useDispatch, useSelector } from "react-redux";
import { addBestDeals } from "../../utils/bestDealsSlice";
import { useEffect } from "react";
import Loading from "../Loading";

const BestDealsPage = () => {
  const { isLoading, bestDeals } = useSelector((store) => store.bestDeals);
  ...
  const getBestDeals = async () => {
    try {
      ...
      dispatch(addBestDeals(res.data.bestDealsCountries));
    } catch (err) {
      console.log("Error Fetching best deals countries : ", err);
    }
  };
  ...
  return (
    <section className=" md:mt-[160px] max-w-[1400px] mx-auto">
      <div className="bg-[url('https://images.unsplash.com/photo-1483304528321-0674f0040030?q=80&w=2340&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D')] bg-cover bg-center h-[200px] flex  items-center justify-center md:mb-[70px] mb-[30px]">
        <h3 className="text-[1.67rem] text-gray-700 font-semibold ">
          Best Deals
        </h3>
      </div>
      <div className="md:flex justify-between  my-[30px] items-center  mt-[50px]">
        <h2 className="md:text-[2rem] text-[1.6rem] font-bold  md:ml-[110px] xl:ml-[150px] pl-[36px] md:pl-[0px] ">
          BEST DEALS{" "}
        </h2>
      </div>
      <div className="grid md:grid-cols-2  lg:grid-cols-2 xl:grid-cols-3 md:mx-[80px] lg:mx-[90px] xl:mx-[120px] gap-[30px]">
        {bestDeals.map((data) => {
          ...
          return (
            <div
              key={_id}
              className="pb-[18px] rounded-lg group overflow-hidden hover:shadow-md  bg-white w-[340px] md:w-[368px] border-[1px] border-gray-200 mb-[40px] mx-auto md:mx-[0px]"
            >
              ...
              <p className="ml-[30px] text-[0.8rem] text-gray-500  ">
                Round-trip, per person
              </p>
              <div className="flex items-center justify-between mr-[30px]">
                <p className="ml-[30px] text-[0.95rem] text-gray-500 mt-[4px] ">
                  Save {discount}%*
                </p>
                <Link to="/flight-booking">
                  <button className="ml-[30px] px-[18px] bg-gray-200 py-[4px] rounded-[6px] text-[0.95rem] text-black font-semibold mt-[4px] hover:text-white hover:bg-blue-400 hover:transition-all hover:duration-300 hover:ease-in-out cursor-pointer">
                    FROM ${price}
                  </button>
                </Link>
              </div>
