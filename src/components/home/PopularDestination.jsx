...
const PopularDestination = () => {
    ...
    const fetchPopularDestinations = async(limit) => {
        try{
            const res = await axios.get(`${BASE_URL}/popular-destinations?limit=${limit}`)
    ...
    <section className="grid sm:grid-cols-2  justify-content md:grid-cols-3 lg:grid-cols-3 xl:grid-cols-4 md:mx-[80px] lg:mx-[100px] ">
        { popularDestinations.length ===0 ? <Loading /> : (
            popularDestinations.map((destination) => {
                const {countryName, city, img, _id, slug} = destination
                return ( <div key={_id} className=" rounded-lg bg-white md:w-[272px] w-[330px]  mx-auto md:mx-[0px] md:border-[1px] md:border-gray-200 mb-[40px] group overflow-hidden hover:shadow-md">
